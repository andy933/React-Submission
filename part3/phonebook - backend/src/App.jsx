...
function App() {
 ...
 /*  const addName = (event) => {
  ...
  const addName = (event) => {
    ...
    const exist = persons.filter(person => person.name === NameObject.name)
    const find = persons.find(person => person.name === NameObject.name)
    // bug for hex id in mongo, const id = persons.indexOf(find)+1 // getindex to change a specific position data
    const person = persons.find(person => person.id === id)
    const changedNumber = {...person, number: newNumber}
    console.log(exist)
    if (exist.length > 0) {
      if (window.confirm(newName + ' is already added to phonebook, replace the old number with a new one?')) {
        personService.update(changedNumber._id, changedNumber)
        .then(returnPersons => {setPersons(persons.map(person => person.id !== id ? person : returnPersons))})
        .catch(error => {
          setMessage(`Information of ${newName} has already been removed from server`)  
          setTimeout(() => {setMessage(null)}, 5000)       
